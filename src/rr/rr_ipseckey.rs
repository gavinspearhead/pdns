...
use crate::dns_helper::{dns_parse_slice, dns_read_u8, names_list, parse_ipv4_addr, parse_ipv6_addr};
...
impl RR_IPSECKEY {
    ...
    pub(crate) fn parse(
        ...
        match ipseckey.gw_type {
            ...
            1 => {
                pk_offset += 4;
                ipseckey.name = parse_ipv4_addr(dns_parse_slice(rdata, 3..7)?)?.to_string();
            } // IPv4 address
            2 => {
                pk_offset += 16;
                ipseckey.name = parse_ipv6_addr(dns_parse_slice(rdata, 3..19)?)?.to_string();
            } // IPv6 Address
            3 => {
