use crate::dns_helper::{
    base32hex_encode, dns_format_name, dns_parse_slice, dns_read_u16, dns_read_u48, NamesList,
};
...
impl RR_TSIG {
    ...
    pub(crate) fn parse(packet: &[u8], offset_in: usize) -> Result<RR_TSIG, ParseError> {
        let mut tsig = RR_TSIG::new();
        let mut pos = offset_in;
        (tsig.name, pos) = dns_parse_name(packet, pos)?;
        tsig.time_signed = dns_read_u48(packet, pos)?;
        pos += 6;
        tsig.fudge = dns_read_u16(packet, pos)?;
        pos += 2;
        let mac_size = usize::from(dns_read_u16(packet, pos)?);
        pos += 2;
        tsig.mac = dns_parse_slice(packet, pos..pos + mac_size)?.to_vec();
        pos += mac_size;
        tsig.orig_id = dns_read_u16(packet, pos)?;
        pos += 2;
        tsig.error = dns_read_u16(packet, pos)?;
        pos += 2;
        let other_len = usize::from(dns_read_u16(packet, pos)?);
        pos += 2;
        tsig.other = dns_parse_slice(packet, pos..pos + other_len)?.to_vec();
        Ok(tsig)
    }
}
